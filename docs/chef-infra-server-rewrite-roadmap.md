...
As of 2026-05-02, OpenCook has moved past pure scaffolding and into the first compatibility and operational hardening slices:
...
- the first migration/cutover tooling slice is now live with redacted preflight checks, OpenCook logical backup create/inspect, offline restore preflight/apply, Chef Server source artifact inventory/normalize/import/sync for normalized source bundles, shadow-read comparison, restored-target complete reindex, cutover rehearsal, and Docker functional coverage
...
- deployment evidence triage is now live through `scripts/deployment-evidence.sh`, which collects redacted smoke, migration, and scale evidence manifests before choosing narrow compatibility or safety follow-up work
- maintenance mode and online repair safety are now live with PostgreSQL-shared maintenance state, process-local standalone truthfulness, Chef-style `503` blocking for mutating Chef-facing writes, preserved reads/read-like POSTs/signed downloads, admin maintenance commands, status/doctor/metrics/log visibility, maintenance-gated reindex/search repair, online default ACL repair, and online org/group/server-admin membership repair through live service seams
...
Use deployment testing, live-source extraction evidence, and operator rehearsal feedback to choose the next compatibility or safety bucket now that PostgreSQL persistence, provider-backed blobs, validator bootstrap, core object persistence, encrypted data bag compatibility, operational admin/reindex/repair tooling, migration/cutover tooling, source import/sync plus shadow-read/cutover hardening, direct live Chef source extraction, maintenance-mode write gating, production-scale migration validation, broader Lucene/query-string compatibility, cookbook/policy/sandbox/checksum negative search compatibility, API-version-specific object semantics, OpenSearch provider capability/version hardening, `chef-server-ctl`-style operational parity, core Chef object compatibility hardening, online repair/admin mutation parity, and deployment evidence triage are pinned.

The recommended next bucket should:

1. Start from real deployment or live-source rehearsal evidence rather than reopening already pinned Chef-facing surfaces speculatively.
2. If compatibility gaps appear, fix the narrow Chef-facing behavior with pedant/live evidence and preserve the completed API-version, auth, search, blob, maintenance, and repair contracts.
3. If operational risk is higher, harden migration safety, rollback evidence, diagnostics, or deployment ergonomics without adding new Chef-facing behavior.
4. Keep broad migration apply/restore/import/sync workflows offline-gated unless a future slice defines an equally narrow live seam and rollback story.
5. Keep licensing, license enforcement, and license telemetry out of scope.

Run `scripts/deployment-evidence.sh smoke`, `scripts/deployment-evidence.sh migration`, or `scripts/deployment-evidence.sh scale` to capture the evidence bundle before selecting that next bucket.

That sequence builds on the completed identity, cookbook/blob, core object, validator bootstrap, active OpenSearch, operational tooling, migration/cutover, source import/sync, live-source extraction, production-scale validation, maintenance-mode, encrypted data bag, Lucene/query-string, API-version, cookbook/policy/sandbox search, provider capability, operational parity, object-compatibility hardening, online repair, and deployment-evidence contracts without reopening their Chef-facing behavior.
