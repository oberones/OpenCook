...
| Sandboxes and cookbooks | Bookshelf, cookbook specs | Partial | `internal/blob`, `internal/api`, `internal/bootstrap` | In progress | Sandbox create/commit flows, signed checksum upload/downloads, cookbook artifact create/read/delete, cookbook version create/update/delete, frozen/force update semantics, broader metadata validation, Chef-shaped filtered/defaulted cookbook read metadata, cookbook read views, and universe responses are live against the in-memory compatibility store; the remaining cookbook pedant cases and production S3-compatible behavior still need follow-on work |
