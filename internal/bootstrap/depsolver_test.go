...
func TestSolveEnvironmentCookbookVersionsIgnoresUnrelatedEnvironmentConstraintsForConflict(t *testing.T) {
	...
	if got := depsolverErr.Detail["message"]; got != "Unable to satisfy constraints on package baz due to solution constraint (foo >= 0.0.0). Solution constraints that may result in a constraint on baz: [(foo = 1.2.3) -> (bar = 1.0.0) -> (baz = 1.0.0), (foo = 1.2.3) -> (buzz = 1.0.0) -> (baz > 1.2.0)]" {
		t.Fatalf("message = %v, want transitive conflict message unchanged by unrelated env constraint", got)
	}
	if got := depsolverErr.Detail["unsatisfiable_run_list_item"]; got != "(foo >= 0.0.0)" {
		t.Fatalf("unsatisfiable_run_list_item = %v, want %q", got, "(foo >= 0.0.0)")
	}
...
func TestSolveEnvironmentCookbookVersionsMatchesUpstreamThirdGraph(t *testing.T) {
	...
	createTestCookbookVersion(t, service, "ponyville", "app1", "0.2.0", map[string]any{
		"dependencies": map[string]any{
			"app2": ">= 0.1.0",
			"app3": ">= 0.1.1",
		},
	...
	createTestCookbookVersion(t, service, "ponyville", "app3", "3.0.0", map[string]any{
		"dependencies": map[string]any{"app5": ">= 2.0.0"},
	}, nil)
	createTestCookbookVersion(t, service, "ponyville", "app3", "4.0.0", map[string]any{
		"dependencies": map[string]any{"app5": ">= 2.0.0"},
	}, nil)
	createTestCookbookVersion(t, service, "ponyville", "app4", "0.1.0", map[string]any{
		"dependencies": map[string]any{"app5": ">= 0.0.0"},
	}, nil)
	createTestCookbookVersion(t, service, "ponyville", "app4", "0.3.0", map[string]any{
		"dependencies": map[string]any{"app5": ">= 0.0.0"},
	...
	createTestCookbookVersion(t, service, "ponyville", "app5", "0.1.0", nil, nil)
	createTestCookbookVersion(t, service, "ponyville", "app5", "0.3.0", nil, nil)
	createTestCookbookVersion(t, service, "ponyville", "app5", "2.0.0", nil, nil)
	createTestCookbookVersion(t, service, "ponyville", "app5", "6.0.0", nil, nil)
...
func TestSolveEnvironmentCookbookVersionsMatchesUpstreamConflictingPassingGraph(t *testing.T) {
	service := newTestBootstrapService(t)
	createTestCookbookOrg(t, service)
	...
	createTestCookbookVersion(t, service, "ponyville", "app4", "0.1.0", map[string]any{
		"dependencies": map[string]any{"app5": "0.1.0"},
	}, nil)
	createTestCookbookVersion(t, service, "ponyville", "app4", "0.3.0", map[string]any{
		"dependencies": map[string]any{"app5": "0.3.0"},
	}, nil)
	createTestCookbookVersion(t, service, "ponyville", "app4", "5.0.0", map[string]any{
		"dependencies": map[string]any{"app5": "2.0.0"},
	}, nil)
...
func TestSolveEnvironmentCookbookVersionsMatchesUpstreamConflictingFailingGraph(t *testing.T) {
	service := newTestBootstrapService(t)
	createTestCookbookOrg(t, service)
	...
	message, ok := depsolverErr.Detail["message"].(string)
	if !ok {
		t.Fatalf("message type = %T, want string", depsolverErr.Detail["message"])
	}
	...
	if got := depsolverErr.Detail["most_constrained_cookbooks"]; len(got.([]string)) != 1 || got.([]string)[0] != "app5 = 2.0.0 -> []" {
		t.Fatalf("most_constrained_cookbooks = %v, want [app5 = 2.0.0 -> []]", got)
	}
}
...
func TestSolveEnvironmentCookbookVersionsSelectsNewerRootVersionWhenEnvironmentAllowsIt(t *testing.T) {
	service := newTestBootstrapService(t)
	createTestCookbookOrg(t, service)
	...
	if solution["foo"].Version != "1.2.3" {
		t.Fatalf("foo version = %q, want %q", solution["foo"].Version, "1.2.3")
	}
	if solution["bar"].Version != "3.0.0" {
		t.Fatalf("bar version = %q, want %q", solution["bar"].Version, "3.0.0")
	}
}
