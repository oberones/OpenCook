...
- the current depsolver slice is now live on `/environments/{env}/cookbook_versions` and the org-scoped alias, with run-list validation, server-side `role[...]` expansion, environment-aware cookbook selection, environment-specific role run-list support, nested-role expansion, version pins, recursive dependency expansion, cookbook-container read auth parity, minimal Chef-style success payloads, datestamp constraint coverage, explicit `400` handling for missing and recursive role inputs, and the current Chef-shaped `400`/`404`/`412` responses for missing, filtered, and dependency-failure cookbook cases
