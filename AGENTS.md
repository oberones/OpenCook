...
- the first role slice:
  ...
  - pinned route semantics for `/roles/{name}/environments` and `/roles/{name}/environments/{environment}` including trailing slashes, method-not-allowed with `Allow: GET`, and extra-path `404`s
