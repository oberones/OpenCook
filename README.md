...
The current role slice is also live on both the default-org and explicit-org routes for `/roles` and `/roles/{name}` plus `/roles/{name}/environments`, `/roles/{name}/environments/{environment}`, and `/environments/{name}/roles/{role}`. Role create/update storage now normalizes and deduplicates top-level `run_list` and `env_run_lists` entries the same way Chef does, so bare cookbook items round-trip back as canonical `recipe[...]` entries on both role GETs and environment-specific role run-list reads. Role environment listings also now preserve linked environment names even when the environment object is missing, while direct reads of those missing environments still return the current Chef-shaped `404`. The role environment routes now also pin trailing-slash acceptance, method-not-allowed responses with `Allow: GET`, extra-path `404`s, and role-read-only auth parity on both default-org and org-scoped paths. Environment-linked role reads now also match the current Chef behavior: `_default` returns the role's top-level run list, named environments return the environment-specific override or `null`, a missing role still wins over a missing environment for `404` precedence, the route authorizes against the role object rather than the environment, and the current route-semantic coverage now also pins missing-organization, ambiguous-default-org, configured-default-org, trailing-slash, method-not-allowed with `Allow: GET`, extra-path `404`, and outside-user `403` behavior.
