...
- configured default-org depsolver resolution is now also explicitly pinned before malformed-request bodies and environment-read auth, including invalid JSON, empty payload, trailing JSON, invalid `run_list`, and malformed-item inputs on both named-environment and `_default` paths
