...
func (s *server) handlePolicies(w http.ResponseWriter, r *http.Request) {
	...
	org, basePath, ok := s.resolvePolicyRoute(w, r, "policies")
	if !ok {
		return
	}
	...
	segments, ok := policyRouteSegments(r.URL.Path, basePath)
	if !ok {
		writeJSON(w, http.StatusNotFound, apiError{
			Error:   "not_found",
	...
	switch len(segments) {
	case 0:
		s.handlePolicyCollection(w, r, state, org, basePath)
	case 1:
...
func (s *server) handlePolicyGroups(w http.ResponseWriter, r *http.Request) {
	...
	org, basePath, ok := s.resolvePolicyRoute(w, r, "policy_groups")
	if !ok {
		return
	}
	if _, exists := state.GetOrganization(org); !exists {
	...
	switch len(segments) {
	case 0:
		s.handlePolicyGroupCollection(w, r, state, org, basePath)
	case 1:
		s.handleNamedPolicyGroup(w, r, state, org, segments[0], basePath)
	case 3:
...
func (s *server) resolvePolicyRoute(w http.ResponseWriter, r *http.Request, root string) (string, string, bool) {
	org := strings.TrimSpace(r.PathValue("org"))
	if org != "" {
		return org, "/organizations/" + org + "/" + root, true
	}
	...
	return org, "/" + root, true
}

func (s *server) handlePolicyCollection(w http.ResponseWriter, r *http.Request, state *bootstrap.Service, org, basePath string) {
	switch r.Method {
	case http.MethodGet:
		...
		writeJSON(w, http.StatusOK, policyListResponse(basePath, policies))
...
func (s *server) handleNamedPolicyRevision(w http.ResponseWriter, r *http.Request, state *bootstrap.Service, org, policyName, revisionID string) {
	...
	case http.MethodHead:
		_, orgExists, policyExists, revisionExists := state.GetPolicyRevision(org, policyName, revisionID)
	...
	default:
		writeMethodNotAllowed(w, "method not allowed for policy revision route", http.MethodGet, http.MethodHead, http.MethodDelete)
	}
}

func (s *server) handlePolicyGroupCollection(w http.ResponseWriter, r *http.Request, state *bootstrap.Service, org, basePath string) {
	switch r.Method {
	case http.MethodGet:
		if !s.authorizeRequest(w, r, authz.ActionRead, authz.Resource{
			Type:         "container",
		...
		writeJSON(w, http.StatusOK, policyGroupListResponse(basePath, groups))
	case http.MethodHead:
		if !s.authorizeRequest(w, r, authz.ActionRead, authz.Resource{
			Type:         "container",
			Name:         "policy_groups",
...
func (s *server) handleNamedPolicyGroup(w http.ResponseWriter, r *http.Request, state *bootstrap.Service, org, groupName, basePath string) {
	switch r.Method {
	case http.MethodGet:
		...
		writeJSON(w, http.StatusOK, namedPolicyGroupResponse(basePath, group))
	case http.MethodHead:
		_, orgExists, groupExists := state.GetPolicyGroup(org, groupName)
		if !orgExists || !groupExists {
			w.WriteHeader(http.StatusNotFound)
			return
		}
		if !s.authorizeRequest(w, r, authz.ActionRead, authz.Resource{
	...
	case http.MethodDelete:
		...
		writeJSON(w, http.StatusOK, namedPolicyGroupResponse(basePath, group))
	default:
		writeMethodNotAllowed(w, "method not allowed for policy group route", http.MethodGet, http.MethodHead, http.MethodDelete)
	}
}
...
func (s *server) handlePolicyGroupAssignment(w http.ResponseWriter, r *http.Request, state *bootstrap.Service, org, groupName, policyName string) {
	...
	case http.MethodGet:
		...
		writeJSON(w, http.StatusOK, revision.Payload)
	case http.MethodHead:
		_, orgExists, groupExists, assignmentExists := state.GetPolicyGroupAssignment(org, groupName, policyName)
		if !orgExists || !groupExists || !assignmentExists {
			w.WriteHeader(http.StatusNotFound)
			return
		}
		if !s.authorizeRequest(w, r, authz.ActionRead, authz.Resource{
	...
	case http.MethodDelete:
		...
		writeJSON(w, http.StatusOK, revision.Payload)
	default:
		writeMethodNotAllowed(w, "method not allowed for policy group assignment route", http.MethodGet, http.MethodHead, http.MethodPut, http.MethodDelete)
	}
}
...
func policyListResponse(basePath string, policies map[string][]string) map[string]any {
...
func policyGroupListResponse(basePath string, groups map[string]bootstrap.PolicyGroup) map[string]any {
...
func namedPolicyGroupResponse(basePath string, group bootstrap.PolicyGroup) map[string]any {
	return map[string]any{
		"uri":      basePath + "/" + group.Name,
