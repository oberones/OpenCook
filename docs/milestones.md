...
- `/roles/{name}/environments` and `/roles/{name}/environments/{environment}` now also pin trailing-slash acceptance, method-not-allowed with `Allow: GET`, extra-path `404`s, and role-read-only auth parity on both default-org and org-scoped routes
