...
| Policies and policy groups | policyfile specs, pedant policy API | Partial | `internal/api`, `internal/bootstrap` | In progress | Default-org `/policies` and `/policy_groups` now support revision create/get/delete, list endpoints, and policy-group assignment flows in memory; broader validation depth, wider route coverage, and PostgreSQL persistence still need follow-on work |
