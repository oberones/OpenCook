...
- the first depsolver slice is now live on `/environments/{env}/cookbook_versions` and the org-scoped alias, with run-list validation, environment-aware cookbook selection, version pins, recursive dependency expansion, cookbook-container read auth parity, minimal Chef-style success payloads, datestamp constraint coverage, and the current Chef-shaped `400`/`404`/`412` responses for missing, filtered, and dependency-failure cookbook cases
- PostgreSQL-backed cookbook persistence, broader cookbook pedant coverage beyond the current environment-filtered/named-filter/latest/version read-write contract, deeper depsolver parity, and deeper operational hardening around provider-backed blob behavior are still pending
