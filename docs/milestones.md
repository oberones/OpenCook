...
- `/roles/{name}/environments` and `/roles/{name}/environments/{environment}` now also pin trailing-slash acceptance, method-not-allowed with `Allow: GET`, and extra-path `404`s on both default-org and org-scoped routes
