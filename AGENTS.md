...
- the `opencook admin` operational surface is live, including OpenCook-to-OpenCook migration preflight, logical backup/restore, normalized Chef Server source inventory/normalize/import/sync, direct read-only live Chef source preflight/extract into normalized bundles, source-to-target shadow-read comparison, restored-target reindex, cutover rehearsal, production-scale fixture generation, scale-aware backup/restore/reindex/shadow-read/cutover functional drills, live-source preflight/extract/import/reindex/shadow/cutover functional drills, operator report summaries, maintenance controls, maintenance-gated reindex/search repair, narrow online default ACL repair, config validation, service status/doctor, metrics, request IDs, structured logs, log discovery, diagnostics bundles, runbook discovery, and service-management docs
- data bag CRUD and encrypted data bag payload opacity are now explicitly pinned as tested compatibility slices
- the current core Chef object compatibility hardening bucket is complete for implemented nodes, environments, roles, data bags, policies, sandboxes, cookbook/blob edge coverage, API-version-specific object reads, auth/ACL no-mutation behavior, active PostgreSQL rehydration, OpenSearch source-of-truth regressions, and Docker `object-compat` coverage
...
- `internal/bootstrap`
  - bootstrap compatibility state, including PostgreSQL-backed core identity/authz persistence and PostgreSQL-backed persistence for implemented core object API state
- `internal/compat`
...
The next likely major slice is broader online repair/admin mutation parity, with deployment-test-discovered or live-source-discovered Chef compatibility gaps taking priority if they prove higher risk.
