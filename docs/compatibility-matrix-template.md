...
| Core Chef objects | `oc_erchef`, pedant object specs | Partial | `internal/api`, `internal/bootstrap`, `internal/store/pg` | In progress | Nodes list/get/head/create/update/delete are live in memory with default-org resolution; wider object coverage and PostgreSQL persistence still need follow-on work |
