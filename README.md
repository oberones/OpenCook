...
The current role slice is also live on both the default-org and explicit-org routes for `/roles` and `/roles/{name}` plus `/roles/{name}/environments` and `/roles/{name}/environments/{environment}`. Role create/update storage now normalizes and deduplicates top-level `run_list` and `env_run_lists` entries the same way Chef does, so bare cookbook items round-trip back as canonical `recipe[...]` entries on both role GETs and environment-specific role run-list reads. Role environment listings also now preserve linked environment names even when the environment object is missing, while direct reads of those missing environments still return the current Chef-shaped `404`.
