	...
	runbooks := []adminRunbook{
		...
		},
		{
			...
			Commands: []adminRunbookCommand{
				...
				{Command: "opencook admin migration source live preflight --source-postgres-dsn DSN --source-bookshelf-root PATH --org ORG --json", Purpose: "validate read-only access to a live Chef PostgreSQL source and local Bookshelf checksum root without source or target mutation"},
				{Command: "opencook admin migration source live extract --source-postgres-dsn DSN --source-bookshelf-root PATH --copy-blobs --org ORG --output live-source --yes --json", Purpose: "write a normalized bundle from implemented live Chef source families and copied checksum blob bytes"},
			...
			Notes: []string{
				...
				"live source preflight and extract use read-only source PostgreSQL access and do not mutate source Chef or the OpenCook target",
