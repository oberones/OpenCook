...
- role environment endpoints are implemented via `/roles/{name}/environments`, `/roles/{name}/environments/{environment}`, and `/environments/{name}/roles/{role}`, including linked-missing-environment list-vs-read parity, Chef-style environment-linked role read behavior, role-read-only auth parity on the environment-linked role route, and pinned route semantics for missing-organization, default-org resolution, trailing-slash, method-not-allowed, extra-path, and outside-user cases
