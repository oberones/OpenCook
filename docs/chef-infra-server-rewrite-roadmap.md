...
- role environment endpoints are implemented via `/roles/{name}/environments`, `/roles/{name}/environments/{environment}`, and `/environments/{name}/roles/{role}`, including linked-missing-environment list-vs-read parity and Chef-style environment-linked role read behavior
