	...
	runbooks := []adminRunbook{
		...
		},
		{
			...
			Commands: []adminRunbookCommand{
				...
				{Command: "opencook admin migration source live preflight --source-postgres-dsn DSN --source-bookshelf-root PATH --org ORG --json", Purpose: "derive and validate separate read-only opscode_chef and bifrost connections plus the local Bookshelf checksum root without source or target mutation"},
				{Command: "opencook admin migration source live extract --source-postgres-dsn DSN --source-bookshelf-root PATH --copy-blobs --org ORG --output live-source --yes --json", Purpose: "write a normalized bundle from independently snapshotted Erchef and Bifrost source data plus copied checksum blob bytes"},
				...
				{Command: "opencook admin migration backup create --output restored-target-backup --offline --yes --json", Purpose: "capture restored target evidence for cutover rehearsal after source import or sync"},
				{Command: "opencook admin migration scale-fixture create --profile medium --output normalized-source --yes --json", Purpose: "generate a deterministic production-scale normalized source bundle for release or operator rehearsal"},
				...
				{Command: "opencook admin migration cutover rehearse --manifest PATH --source normalized-source --source-import-progress source-import-progress.json --source-sync-progress source-sync-progress.json --search-check-result PATH --shadow-result PATH --maintenance-result maintenance-status.json --source-frozen --rollback-ready --server-url URL --json", Purpose: "gate client cutover using source-freeze, import, sync, search, shadow-read, maintenance, auth, blob, and rollback evidence"},
				{Command: "OPENCOOK_FUNCTIONAL_SCALE_PROFILE=medium scripts/functional-compose.sh migration-scale-all", Purpose: "run the opt-in production-scale Compose drill with generated fixtures and operator reports"},
			},
			Notes: []string{
				"migration JSON includes an operator_report section with inventory totals, findings, evidence gates, retry guidance, and next steps",
				"live source preflight and extract derive separate read-only Erchef and Bifrost database connections from the cluster seed DSN and do not mutate source Chef or the OpenCook target",
				"the two database snapshots are not cross-database atomic; final cutover requires external source-freeze evidence",
				"use --copy-blobs with a local Bookshelf root for self-contained bundles; use --reference-blobs only when separate provider reachability and checksum validation are planned before cutover",
				"treat source DSNs, blob URLs, source private keys, signed URLs, and provider responses as secrets even though command output redacts known credential fields",
				...
				"for emergency rollback, route clients back to source Chef, keep OpenCook target writes frozen, and preserve failed migration reports before retrying",
		...
		},
		{
			...
			Unsupported: []adminRunbookUnsupportedEntry{
				...
				{Workflow: "omnibus maintenance-mode wrappers", Reason: "OpenCook has its own maintenance-mode traffic blocking gate; chef-server-ctl-specific wrapper behavior remains outside the product surface"},
