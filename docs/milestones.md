...
- `/roles/{name}/environments`, `/roles/{name}/environments/{environment}`, and `/environments/{name}/roles/{role}` are live, including `_default` run-list resolution plus Chef-style environment-linked role reads
