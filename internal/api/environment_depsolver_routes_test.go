...
func TestEnvironmentCookbookVersionsRejectMalformedRunListItemBeforeEnvironmentReadAuthz(t *testing.T) {
	routes := []struct {
		name    string
		path    string
		envName string
	...
	for _, route := range routes {
		t.Run(route.name, func(t *testing.T) {
			...
			router.ServeHTTP(rec, req)
			if rec.Code != http.StatusBadRequest {
				t.Fatalf("%s malformed run_list item precedence status = %d, want %d, body = %s", route.name, rec.Code, http.StatusBadRequest, rec.Body.String())
			}
...
func TestEnvironmentCookbookVersionsRejectMalformedRunListItemBeforeMissingEnvironment(t *testing.T) {
	routes := []struct {
		name string
		path string
	}{
	...
	for _, route := range routes {
		t.Run(route.name, func(t *testing.T) {
			...
			newTestRouter(t).ServeHTTP(rec, req)
			if rec.Code != http.StatusBadRequest {
				t.Fatalf("%s malformed run_list item before missing environment status = %d, want %d, body = %s", route.name, rec.Code, http.StatusBadRequest, rec.Body.String())
			}
...
func TestEnvironmentCookbookVersionsRejectEmptyPayloadBeforeEnvironmentReadAuthz(t *testing.T) {
	routes := []struct {
		name    string
		path    string
		envName string
	...
	for _, route := range routes {
		t.Run(route.name, func(t *testing.T) {
			...
			req := newSignedJSONRequest(t, http.MethodPost, route.path, []byte(""))
			rec := httptest.NewRecorder()
			router.ServeHTTP(rec, req)
			if rec.Code != http.StatusBadRequest {
				t.Fatalf("%s empty payload precedence status = %d, want %d, body = %s", route.name, rec.Code, http.StatusBadRequest, rec.Body.String())
			}
...
func TestEnvironmentCookbookVersionsRejectEmptyPayloadBeforeMissingEnvironment(t *testing.T) {
	routes := []struct {
		name string
		path string
	}{
	...
	for _, route := range routes {
		t.Run(route.name, func(t *testing.T) {
			...
			newTestRouter(t).ServeHTTP(rec, req)
			if rec.Code != http.StatusBadRequest {
				t.Fatalf("%s empty payload before missing environment status = %d, want %d, body = %s", route.name, rec.Code, http.StatusBadRequest, rec.Body.String())
			}
